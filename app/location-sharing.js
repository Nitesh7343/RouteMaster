...
export default function LocationSharingScreen() {
  ...
  const saveLocationToMongoDB = async (locationData) => {
    try {
      ...
      if (!driverId || !routeId || !token) {
        console.warn('Missing driverId/routeId/token');
        return;
      }
      ...
      // Send to backend
      await api.post('/api/driver/location', locationDocument, {
        headers: { Authorization: `Bearer ${token}` },
      });

      console.log('Location sent:', locationDocument);
    } catch (error) {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.statusContainer}>
          <Text style={styles.statusText}>
            Status:{' '}
            {isSharing
              ? 'ON - Sharing coordinates to database'
              : 'OFF - Location sharing stopped'}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  toggleContainer: { alignItems: 'center', marginBottom: 40 },
  toggleButton: {
    width: 120, height: 120, borderRadius: 60, justifyContent: 'center', alignItems: 'center',
    elevation: 5, shadowColor: '#000', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.25, shadowRadius: 3.84,
  },
  toggleButtonOn: { backgroundColor: '#4CAF50' },
  toggleButtonOff: { backgroundColor: '#fff', borderWidth: 3, borderColor: '#007AFF' },
  toggleButtonLoading: { opacity: 0.7 },
  toggleText: { fontSize: 24, fontWeight: 'bold' },
  toggleTextOn: { color: '#fff' },
  toggleTextOff: { color: '#007AFF' },
  locationInfo: {
    backgroundColor: '#fff', padding: 20, borderRadius: 10, marginBottom: 20, width: '100%',
    elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.22, shadowRadius: 2.22,
  },
  locationTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 10 },
  locationText: { fontSize: 14, color: '#666', marginBottom: 5 },
  statusContainer: { backgroundColor: '#fff', padding: 15, borderRadius: 8, marginBottom: 30, width: '100%' },
  ...
  doneButton: {
    backgroundColor: '#4CAF50', paddingHorizontal: 30, paddingVertical: 15,
    borderRadius: 8, alignItems: 'center',
  },
